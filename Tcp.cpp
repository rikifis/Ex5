...
***********************************************************************/
int Tcp::receiveData(char* buffer, int size, int socketDesc) {
	int read_bytes = recv(this->isServer ? this->descriptorCommunicateClient
			              : socketDesc, buffer, size, 0);
	//checking the errors
	if (read_bytes == 0) {
